...
sudo netstat -tlnp | grep :8000 || echo "❌ Нічого не слухає на порту 8000"
    ...
    sudo ss -tlnp | grep :8000 || echo "❌ Нічого не слухає на порту 8000"
else
    echo "⚠️  netstat/ss не знайдено, перевірка через lsof..."
    sudo lsof -i :8000 || echo "❌ Нічого не слухає на порту 8000"
fi
echo ""
...
sudo netstat -tlnp | grep :8000 && echo "✅ Порт відкритий" || echo "❌ Порт закритий"
    ...
    sudo ss -tlnp | grep :8000 && echo "✅ Порт відкритий" || echo "❌ Порт закритий"
else
    sudo lsof -i :8000 && echo "✅ Порт відкритий" || echo "❌ Порт закритий"
fi
echo ""
