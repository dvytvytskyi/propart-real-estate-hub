...
echo "⚡ ПОВНЕ ВИМКНЕННЯ АВТОМАТИЧНИХ ВИКЛИКІВ HUBSPOT"
echo "=========================================="
...
# 2. Перевірити чи є інші автоматичні виклики HubSpot в routes
echo ""
echo "🔍 Перевірка інших автоматичних викликів HubSpot..."

# Перевірка чи є виклики в dashboard (має бути вже вимкнено)
if grep -q "update_hubspot_stage_labels_for_leads" app.py | grep -v "^[[:space:]]*#"; then
    echo "⚠️  Знайдено активний виклик update_hubspot_stage_labels_for_leads"
    echo "   Коментую..."
    sed -i 's/^\([[:space:]]*\)\(updated_count = update_hubspot_stage_labels_for_leads\)/\1# ⚡ ВИМКНЕНО: \2/' app.py
fi
...
ps aux | grep -E "[g]unicorn" | head -5
